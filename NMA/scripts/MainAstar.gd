...
@onready var destIcon = preload("res://NMA/scenes/landMark.tscn").instantiate()
...
var interOrigin = GlobalsNMA.interfloor_origin
var InterDest = GlobalsNMA.interfloor_destination
var sourceGroup = GlobalsNMA.source_group
...
func _ready():
	...
	GlobalsNMA.recent_scene = get_tree().current_scene.scene_file_path
	GlobalsNMA.gallery_sourcegroup = originGroup
	GlobalsNMA.floor_name = originGroup
	StaticDatabaseNMA.current_galleries = galleries
	%Greetings.text = "National Museum of Anthropology"
	...
	if GlobalsNMA.see_on_map_gallery != null:
...
func minPath(nearestGroup):
	...
	GlobalsNMA.interfloor_origin = shortestDest
...
func _on_start_buttons_nma_2f_button_pressed(buttonName):
...
func _on_start_buttons_nma_3f_button_pressed(buttonName):
	startScroll.visible = false
	startLine.text = buttonName
...
func _on_start_buttons_nma_4f_button_pressed(buttonName):
	startScroll.visible = false
	startLine.text = buttonName
...
func _on_destinations_nma_button_pressed(buttonName):
	...
	for loc in destLocs.get_children():
		if loc.name == buttonName:
			if loc.is_in_group(originGroup) == false:
				...
				GlobalsNMA.interfloor_destination = loc
...
func _on_directions_pressed():
	...
	if isInOriginGroup == false:
		...
		drawPath()
		camTween()
		GlobalsNMA.interfloor_destination = GlobalsNMA.interfloor_destination.name
		GlobalsNMA.interfloor_origin = GlobalsNMA.interfloor_origin.name
		GlobalsNMA.source_group = originGroup
